...
from ta.trend import IchimokuIndicator, PSARIndicator, SMAIndicator, MACD, ADXIndicator
...
class EnhancedETHTradingBot:
    def __init__(
        ...
        risk_per_trade: float = 1,  # Usar el 100% del balance
        ...
        self.set_leverage(10)  # Forzamos apalancamiento a 10
    ...
    def set_leverage(self, leverage: int = 10):
        ...
        try:
            ...
            logger.info(f"Apalancamiento configurado: {response}")
        except Exception as e:
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # Trend: parámetros posicionales para Ichimoku
    ...
    def calculate_position_size(self, price: float, stop_loss: float) -> float:
        """
        Calcula el tamaño de posición usando el balance completo (si risk_per_trade es 1)
        y luego limita el tamaño máximo según el margen disponible con apalancamiento 10.
        """
        try:
            ...
            # Se usa el risk_per_trade (1 para usar el 100% del balance)
            ...
            max_size = balance * 10 / price  # apalancamiento forzado a 10
    ...
    def open_position(self, side: str, price: float, atr: float):
        ...
        Para LONG: orden BUY, SL = precio - (atr*1.5) y TP = precio + (atr*3).
        Para SHORT: orden SELL, SL = precio + (atr*1.5) y TP = precio - (atr*3).
        """
        if side == 'LONG':
        ...
        # Formatear la cantidad según la precisión permitida
        decimals = int(-np.log10(self.lot_size))
        quantity_str = format(size, f".{decimals}f")
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if order['status'] == 'FILLED':
                    pnl = (price - pos['entry']) * size if pos['side'] == 'LONG' else (pos['entry'] - price) * size
        ...
        if signal == 'BUY':
            if self.symbol in self.positions:
                if self.positions[self.symbol]['side'] == 'SHORT':
                    ...
                    # Actualizar trailing stop o no hacer nada si ya está LONG
                    pass
            else:
                self.open_position('LONG', price, atr)
        elif signal == 'SELL':
            if self.symbol in self.positions:
                if self.positions[self.symbol]['side'] == 'LONG':
                    ...
                    # Actualizar trailing stop o no hacer nada si ya está SHORT
                    pass
            else:
                self.open_position('SHORT', price, atr)
...
if __name__ == "__main__":
    try:
        ...
        bot = EnhancedETHTradingBot(
            ...
            symbol='ETHUSDT',
            telegram=telegram,
            risk_per_trade=1,  # Usar el 100% del balance
