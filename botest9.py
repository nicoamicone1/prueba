...
PARAMETROS = {
    ...
    'duracion_maxima_operacion': 300,  # 5 minutos máximo
    'min_trade_duration': 300,         # Mínimo 5 minutos antes de ajustar órdenes
    'intervalo_actualizacion': 60,       # Chequeo de señales cada 1 minuto
    'intervalo_actualizacion_modelo': 300,  # Reentrenamiento del modelo cada 5 minutos
...
def ejecutar_orden(simbolo, direccion, precio, df):
    ...
    if direccion == 'BULLISH':
        SL = precio - atr
        TP = precio + atr  # Se puede ajustar el multiplicador para un mejor ratio riesgo/recompensa
...
# ===========================
# FUNCIONES DINÁMICAS DE AJUSTE DE ÓRDENES (TRAILING)
# ===========================
def actualizar_ordenes_dinamicamente():
    """
    Si hay posición abierta y el trade ha estado activo al menos el tiempo mínimo,
    recalcula niveles de SL y TP basados en ATR y precio actual,
    y actualiza (reemplaza) las órdenes de protección sin cerrar la posición.
    """
    global trade_entry_time
    try:
        # Verificar si se cumple el tiempo mínimo de trade
        if trade_entry_time is not None and (time.time() - trade_entry_time < PARAMETROS['min_trade_duration']):
            print(f"{COLORES['warning']}Trade abierto por menos de {PARAMETROS['min_trade_duration']} segundos. No se actualizan las órdenes.")
            return
