...
PARAMETROS = {
    ...
    'notional_minimo': 20,
    # Usamos porcentajes fijos para stop loss y take profit
    'stop_loss_pct': 0.01,      # 1% de pérdida
    'profit_target_pct': 0.02,  # 2% de ganancia objetivo
    ...
    'umbral_rsi_venta': 55,
    'umbral_macd': 0.1,
    'trailing_factor': 0.5  # factor para trailing (si se quiere implementar)
}
...
def actualizar_ordenes_dinamicamente():
    ...
    try:
        if trade_entry_price is None:
            print(f"{COLORES['error']}❌ No entry price")
            return
        ...
        # Usar el precio de entrada original para calcular SL y TP
        ...
        print(f"{COLORES['success']}✅ Órdenes ajustadas | Precio Entrada: {precio_entrada:.2f} | SL: {SL:.2f} | TP: {TP:.2f}")
    except Exception as e:
    ...
    try:
        ...
        for tf_interval, peso in zip(TIMEFRAMES, PESOS):
            ...
            if df is not None:
                ...
                # Análisis LSTM con umbral del 2%
...
def ejecucion_principal():
    ...
    try:
        while True:
            ...
            # Para señales bullish usamos el precio ask, y para bearish el bid
            ...
            if señal != 'NEUTRAL' and precio is not None:
                ...
                if posicion is None:
                    print(f"{COLORES['success']}🚀 Señal detectada: {señal} | Precio: {precio:.2f} (Sin posición abierta)")
                    cancelar_todas_las_ordenes()
